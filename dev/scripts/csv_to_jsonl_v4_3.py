...
class CSVToJSONLConverter:
    ...
    def _safe_strip(self, value) -> str:
        """Safely strip a value, handling various data types"""
        if value is None:
            return ""
    ...
    def stream_csv_rows(self, csv_path: str) -> Iterator[Dict[str, Any]]:
        """
        Memory-efficient streaming of CSV rows with improved error handling
        """
        try:
            with open(csv_path, 'r', encoding='utf-8', newline='') as csvfile:
                ...
                # Try different approaches for problematic files
                try:
                    reader = csv.DictReader(csvfile, delimiter=delimiter)
                    fieldnames = reader.fieldnames

                    for row_num, row in enumerate(reader, 1):
                        # Clean row data with safe stripping
                        cleaned_row = {}
                        for k, v in row.items():
                            if k is not None:  # Skip None keys
                                cleaned_row[k] = self._safe_strip(v)

                        yield row_num, cleaned_row, fieldnames

                except (csv.Error, UnicodeDecodeError) as e:
                    print(f"CSV parsing error for {csv_path}: {e}")
                    # Try alternative parsing approach
                    csvfile.seek(0)
                    try:
                        # Read as regular CSV without DictReader first
                        ...
                        for row_num, row in enumerate(reader, 1):
                            ...
                            for i, value in enumerate(row):
                                if i < len(header):
                                    ...
                                    cleaned_row[key] = self._safe_strip(value)

                            yield row_num, cleaned_row, header

                    except Exception as fallback_error:
                        print(f"Fallback parsing also failed for {csv_path}: {fallback_error}")
                        return
    ...
    def convert_to_jsonl(self, csv_path: str, jsonl_path: str,
        ...
        # Handle the max_rows formatting properly
        if max_rows is None:
            print("Max rows: unlimited")
        else:
    ...
    def _convert_row_to_json(self, row: Dict[str, str], schema: str, mapping: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        if schema == 'alpaca':
            instruction = self._safe_strip(row.get(mapping['instruction'], ''))
            output = self._safe_strip(row.get(mapping['output'], ''))
            ...
            json_obj = {
                ...
                'input': self._safe_strip(row.get(mapping['input'], '')) if mapping.get('input') else '',
                'output': output
            }
            return json_obj

        elif schema == 'qa':
            question = self._safe_strip(row.get(mapping['question'], ''))
            answer = self._safe_strip(row.get(mapping['answer'], ''))
            ...
            text = self._safe_strip(row.get(mapping['text'], ''))
            if not text:
                return None
            return {'text': text}
            ...
            for json_key, csv_column in mapping.items():
                if csv_column in row:
                    value = self._safe_strip(row[csv_column])
...
def main():
    ...
    if args.max_rows is None:
        print("Max rows per file: unlimited")
    else:
