...
This project is still ongoing. I lost access to my medication for a period of time, and I am now sorting through and picking up the threads of the work. A lot has changed since then, so parts of this project may need to be revised or discarded.
...
- I included a README for each file or dataset once I remembered to add one.
- The original prompts used to create the dataset files are included under `DEV` with the scripts.

## Repository contents

- `DEV/` — scripts used to convert the original CSV files to JSONL, along with the original prompts.
- Dataset files — the generated training data and supporting files.
